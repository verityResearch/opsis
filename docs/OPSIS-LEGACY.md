# Legacy Opsis Prototype

This is the historical explanation of the `ROAM/1 opsis` prototype, not the
[standalone ROAMV specification](../spec/roamv/FORMAT.md). Its fixed token
estimates, inability-to-watch claim, travel-only selection, and unaltered-frame
claims are not general format guarantees. The [audit and plan](ROAMV-PLAN.md)
record concrete limitations. The legacy implementation has not been repaired or
made ROAMV-conformant by the standalone draft.
...
The proposed video extension is described in the [ROAMV design draft](ROAMV-DESIGN.md),
with research references, source and timing contracts, sampling policies, and reader behavior.
The [implementation plan](ROAMV-PLAN.md) records current counterexamples and staged
validation. ROAMV is a proposal; this repository currently implements the opsis primitive.
